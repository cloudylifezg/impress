...
def z_encode(p):
    ...
    if p == None:                               #None->PHP中的NULL
        ...
        p_bytes = p#.decode(php_python.CHARSET);
        ret = 's:%d:"' % len(p_bytes)
        ret = ret#.decode(php_python.CHARSET)
        ret = ret + p_bytes + '";'#.decode(php_python.CHARSET)
        ret = str(ret).decode(php_python.CHARSET)
        return ret
